...
STAGES = [
    ("cw-render-gap", "probe_render_gap.py", [], True),
    ("cw-schema-truth", "probe_schema_truth.py", [], True),
    ("cw-quotability", "probe_quotability.py", [], True),
    ("cw-time-decay", "probe_time_decay.py", [], True),
    ("cw-entity-consensus", "probe_entity_consensus.py", ["--phase", "onsite"], True),
    ("cw-arrival-experience", "probe_arrival.py", [], True),
]
...
def main():
    ...
    ap.add_argument("--budget-seconds", type=int, default=150, help="Crawl budget (network stage)")
    ...
    crawl_dir = os.path.join(SKILLS, "cw-reach-gate", "scripts")
    ...
    log.append({"stage": "cw-reach-gate", **result})
    ...
    if args.merge_probes:
        script_dir = os.path.join(SKILLS, "cw-entity-consensus", "scripts")
        result = run([python, os.path.join(script_dir, "probe_entity_consensus.py"),
                      "--workspace", workspace, "--phase", "merge"],
                     cwd=script_dir, timeout=180)
        log.append({"stage": "cw-entity-consensus (merge)", **result})
    ...
    log.append({"stage": "cw-audit-conductor", **result})
    if result["returncode"] != 0:
