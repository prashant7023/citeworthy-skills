...
description: >-
  ...
  Compares raw server HTML against the rendered DOM when a browser is available
  and degrades to explicitly-labelled heuristics when it is not. Use when a page
...
- **Renderer available** — the raw-vs-rendered word-count delta is *measured*. Findings
  are `high` confidence and cite both numbers.
- **No renderer** — no delta is invented. The skill falls back to a conservative
  signature (an empty app-shell element and/or a framework bundle, **plus** under 150
  words of server-rendered text) and reports at `medium`/`low` confidence with an
  explicit `verification` instruction.
...
10. Write `findings/cw-render-gap.json`.
...
`findings/cw-render-gap.json`, pillar `parse`. Findings carry `confidence` and,
